...
algorithm based on the paper methodology, with improvements for robustness and 
...
class IntrinsicDimensionEstimator:
    ...
    Attributes:
        ...
        distance_metric (str): Distance metric to use ('l1' or 'l2', default: 'l1')
    ...
    def __init__(self, 
        ...
        logger.info(f"Initialized IntrinsicDimensionEstimator with {distance_metric} distance, "
    ...
    def estimate(self, data: np.ndarray) -> Tuple[int, int, float]:
        ...
        if original_size > self.max_samples:
            logger.warning(f"Dataset is large ({original_size} samples). "
    ...
    def _compute_pairwise_distances(self, data: np.ndarray) -> np.ndarray:
        ...
        logger.debug(f"Computed {len(distances)} valid distances, "
                    f"range: [{np.min(distances):.6f}, {np.max(distances):.6f}]")
        
        return distances
    
    def _estimate_from_correlation_function(self, distances: np.ndarray, 
                                          original_dims: int) -> int:
        """
        ...
        try:
            ...
            logger.warning("Error in log spacing, using linear spacing")
    ...
    def _calculate_gradients(self, correlation_values: np.ndarray, 
    ...
    def _calculate_log_log_gradients(self, correlation_values: np.ndarray,
                                   radii: np.ndarray) -> np.ndarray:
    ...
    def _select_intrinsic_dimension(self, gradients: np.ndarray, 
                                  log_gradients: np.ndarray,
                                  original_dims: int) -> int:
        """
        ...
        """
        logger.debug(f"Selecting intrinsic dimension from gradients. "
    ...
    def _estimate_for_config_dataset(self, gradients: np.ndarray, 
                                   log_gradients: np.ndarray, 
                                   original_dims: int) -> int:
    ...
    def _estimate_for_behavior_dataset(self, gradients: np.ndarray,
                                     log_gradients: np.ndarray,
                                     original_dims: int) -> int:
    ...
    def _estimate_for_medium_dataset(self, gradients: np.ndarray,
                                   log_gradients: np.ndarray, 
                                   original_dims: int) -> int:
