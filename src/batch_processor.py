...
class BatchProcessor:
    ...
    def __init__(self, 
        ...
        logger.info(f"Initialized BatchProcessor: results={results_file}, "
                   f"errors={error_log_file}")
    
    def process_datasets_from_file(self, datasets_file: str,
    ...
    def _initialize_results_file(self):
        ...
        if not os.path.exists(self.results_file):
            headers = [
                'Dataset', 'IS LITE == DEHB', 'same', 'Separator', 'Type',
                'Raw', 'Intrinsic', 'DRR = 1 - (I/R)', 'DRR vs Agrawal', 'IS LITE == DEHB'
            ]
    ...
    def _process_single_dataset(self, config: Dict[str, str], 
                               data_root: str) -> bool:
    ...
    def _save_result_to_csv(self, dataset_name: str, original_dims: int,
                           intrinsic_dim: int, drr: float):
        """Save processing result to CSV file."""
        result_row = [
            dataset_name, '', '', '', '',  # Empty columns for compatibility
            original_dims, intrinsic_dim, f"{drr:.3f}", '', ''
        ]
    ...
    def _log_error(self, dataset_name: str, error_message: str, 
