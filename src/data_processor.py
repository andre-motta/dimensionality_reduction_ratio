...
class DataProcessor:
    ...
    def __init__(self, max_rows_for_processing: int = 5000, random_seed: int = 42):
        ...
        logger.info(f"Initialized DataProcessor with max_rows={max_rows_for_processing}, "
                   f"seed={random_seed}")
    ...
    def process_dataset(self, data: Union[pd.DataFrame, np.ndarray, str]) -> Tuple[np.ndarray, dict]:
        ...
        if data.shape[0] > self.max_rows_for_processing:
            logger.warning(f"Dataset is very large ({data.shape[0]} rows). "
    ...
    def _process_columns(self, data: pd.DataFrame, metadata: dict) -> pd.DataFrame:
        ...
        logger.info(f"Processed {len(metadata['numeric_columns'])} numeric and "
                   f"{len(metadata['categorical_columns'])} categorical columns")
        
        return data_processed
    
    def _try_numeric_conversion(self, data: pd.DataFrame, column: str, 
                               col_data: pd.Series) -> bool:
        """
    ...
    def _handle_categorical_column(self, data: pd.DataFrame, column: str,
                                  col_data: pd.Series) -> bool:
        """
    ...
    def _finalize_processing(self, data: pd.DataFrame, metadata: dict) -> np.ndarray:
        ...
        logger.info(f"Final data quality check: shape={data.shape}, "
